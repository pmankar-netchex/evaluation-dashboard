...
interface CaseLoaderProps {
  onLoadCase: (identifier: string, type: 'case' | 'messagingSession' | 'messagingSessionName') => void;
  loading?: boolean;
...
export function CaseLoader({
  ...
  const [inputType, setInputType] = useState<'case' | 'messagingSession' | 'messagingSessionName'>('case');
  ...
  return (
    <div className="bg-white border border-[#eeeeee] rounded-lg p-6 shadow-sm">
        ...
        <div className="flex flex-wrap gap-4">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              onChange={(e) => setInputType(e.target.value as 'case' | 'messagingSession' | 'messagingSessionName')}
              disabled={loading}
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="radio"
              name="inputType"
              value="messagingSessionName"
              checked={inputType === 'messagingSessionName'}
              onChange={(e) => setInputType(e.target.value as 'case' | 'messagingSession' | 'messagingSessionName')}
              disabled={loading}
              className="w-4 h-4 text-[#2196f3] focus:ring-[#2196f3]"
            />
      ...
      {(currentCaseNumber !== undefined || currentMessagingSessionId || currentMessagingSessionName) && (
          ...
          <div className="text-sm font-normal leading-5 text-[#757575]">
            <div>
              ...
              {currentCaseNumber ? (
                ...
                <span className="text-[#9e9e9e] italic">No Case Id</span>
              )}
            </div>
            {currentMessagingSessionId && (
              <>
